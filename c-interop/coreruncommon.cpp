	...
	const char* managedAssemblyAbsolutePath)
{
	...
	if (coreclrLib != nullptr)
	{
		...
		else
		{
			...
			exitCode = initializeCoreCLR(
				currentExeAbsolutePath,
				"messageheighttwitch-c-interop",
...
extern "C" CalculateMessageHeight *CreateCalculateMessageHeightDelegate()
{
	...
	if (!SUCCEEDED(st))
	{
		fprintf(stderr, "CreateCalculateMessageHeightDelegate failed - status: 0x%08x\n", st);
		return NULL;
	}

	return (CalculateMessageHeight*)calc;
}
